...
-- Run this once in SQL Editor of your new project (order matters).

-- ===== 20260731080921_2d331778-38eb-45df-9051-749bc9652b36.sql =====
...
-- ===== 20260731080953_ba9e7ee6-2e30-4176-858a-102631d0a02e.sql =====
...
-- ===== 20260731081033_8c3ac741-9f9d-400f-9a96-fe3c28b9bd39.sql =====
...
CREATE POLICY "admin read product images" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'product-images' AND public.is_admin());

-- Grant the verified store owner permanent admin access. This is tied to the
-- authenticated user identity, not to a domain, browser, or stored password.
...
FOR EACH ROW
EXECUTE FUNCTION public.grant_primary_admin_for_verified_email();
